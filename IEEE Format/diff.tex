...
%DIF DEL old.tex   Tue Oct 31 14:44:30 2017
%DIF ADD new.tex   Fri Nov  3 14:29:46 2017
...
\DIFdelbegin %DIFDELCMD < \hl{ The objective was to use a modified PN guidance with uncertain position information to reduce the distance to target, referred to as intercept, and to tail chase the target, referred to as follow.}
%DIFDELCMD < %%%
\DIFdelend 



%DIF < \hl{ PN guidance was used to reduce the distance between a target and interceptor, referred to as intercept, and to tail chase the target, referred to as follow.}
...
\DIFdel{. }%DIFDELCMD < \hl{East is the positive $X$ axis and up, or altitude, is the positive $Y$ axis.} %%%
...
\DIFadd{\phi }\DIFaddend = \DIFdelbegin \DIFdel{V_{T} }%DIFDELCMD < \begin{bmatrix} cos(\beta) \\ sin(\beta) \end{bmatrix}
%DIFDELCMD < %%%
...
\cite{zarchan}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Conservative gains \DIFdelbegin \DIFdel{lead to }\DIFdelend \DIFaddbegin \DIFadd{result in }\DIFaddend larger turn radii \DIFdelbegin \DIFdel{and make for a less maneuverable interceptor. Higher }\DIFdelend \DIFaddbegin \DIFadd{whereas larger }\DIFaddend gains result in \DIFdelbegin \DIFdel{a more maneuverable vehicle as long as the commanded heading changes are not beyond the vehicles physical capabilities. }%DIFDELCMD < \hl{Multi-rotors are capable of making abrupt heading changes in comparison to fixed wing UAVs and it was assumed a guidance gain of $N = 5$ is not beyond a multi-rotors capabilities. The resulting guidance is expected to return sharp turns and straight line optimal paths when used with the Dubins kinematic model.} %%%
\DIFdel{The LOS rate is provided by Equation \ref{eq:losrate}, which requires the relative position and velocity components $R_{TIx}$,$R_{TIy}$, and $V_{TIx}$, $V_{TIy}$ shown in Equations \ref{eq:pos} and \ref{eq:vel} }\DIFdelend \DIFaddbegin \DIFadd{smaller turn radii. }\hl{Multi-rotors are capable of making abrupt heading changes in comparison to fixed wing UAVs and it was assumed a guidance gain of $N = 5$ is not beyond multi-rotor capability. The resulting guidance is expected to return sharp turns and straight line optimal paths when used with the Dubins kinematic model.} \DIFadd{The LOS angle and rate are calculated in Equations \ref{eq:LOS} and \ref{eq:losrate} }\DIFaddend respectively. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=7cm] {\DIFdelbeginFL \DIFdelFL{x50}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{failedHeadOn}\DIFaddendFL }
...
\DIFdelbegin \DIFdel{For a tail-chase scenario the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend interceptor does not fail to intercept and follow the target \DIFaddbegin \DIFadd{for a tail-chase scenario }\DIFaddend shown in Figure \ref{fig:successfulFollow} and no singularities are present in Figure \ref{fig:successfulFollowLOS}. The tail-chase scenario has the added benefit of placing the interceptor behind the target, allowing for \DIFdelbegin \DIFdel{an easy }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend transition into a follow. The elimination of the singularity and easy transition into a follow make a case for waiting on the target before intercepting it directly, which is the motivation for introducing a pseudotarget into the PN guidance model.

\begin{figure}[H]
	\centering
	\includegraphics[width=7cm] {\DIFdelbeginFL \DIFdelFL{x15}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{tailChaseWithLedgend}\DIFaddendFL }
...
Measuring the LOS rate and range \DIFdelbegin \DIFdel{would be done by on-board sensors which would have some uncertainty $r$. Uncertainties }\DIFdelend \DIFaddbegin \DIFadd{uncertainties }\DIFaddend in $x$ and $y$ are assumed to be equal so the actual position of the target is in the space $(x_T\pm r,y_T \pm r)$\DIFaddbegin \DIFadd{, where $r$ is the radius of uncertainty}\DIFaddend . Figure \ref{fig:uncertrad} shows a representation of the measurement as a point centered in a circle of radius $r$. The region inside the circle is the space where the target may actually exist. The interceptor should avoid the inside of the circular region to prevent an unintended collision with the target. The uncertainty decreases linearly as a function of range as shown in Equations \ref{eq:uncert} and \ref{beq}.
...
\begin{figure}[H]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7cm]{step1}
%DIFDELCMD < 	%%%
...
\begin{figure}[H]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7cm]{step2}
%DIFDELCMD < 	%%%
...
\begin{figure}[H]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7cm]{step3}
%DIFDELCMD < 	%%%
...
\begin{figure}[H]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7cm]{rinit07.png}
%DIFDELCMD < 	%%%
...
\begin{figure}[H]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7cm]{rinit08.png}
%DIFDELCMD < 	%%%
...
\begin{figure}[H]
	\centering
	\includegraphics[width=7cm] {\DIFdelbeginFL \DIFdelFL{ptx50}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fixedSingularity}\DIFaddendFL }
	\caption{\DIFdelbeginFL \DIFdelFL{Initial target position $x_T=50$ ending in tail }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Tail }\DIFaddendFL chase \DIFaddbeginFL \DIFaddFL{with no singularities, and steady state range $R_{TI}\approx0$}\DIFaddendFL }
